...
contract Polybuccaneers is ERC721Enumerable, Ownable {
  using Strings for uint256;
...
// The bonding curve, to enable early adopters to buy cheap, and raising funds for next steps
  ...
  function mint(uint256 _mintAmount) public payable {
    ...
    if (msg.sender != owner()) {
        if(onlyWhitelisted == true) {
            ...
            require(ownerTokenCount  + _mintAmount  <= nftPerAddressLimit, "Max buy for presale exceeded!");
        }
 ...
 // Give a random NFT to a contest winner
  ...
  function tokenURI(uint256 _id) public view virtual override returns (string memory){
    require(_exists(_id),"Fantoms: URI query for nonexistent token");
